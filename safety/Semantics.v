...
Lemma sem_eval_well_defined' : 
  forall (decls:Decls) (ctx:Context) (mu:Memory) (e:Expr) (t:Tuple),
    ...
    HasType ctx e t ->
      forall (i:Tuple), i.<=.t -> eval ctx mu e i = VSome.
Proof with auto.
  intros decls ctx mu e t CtxComp MemComp HT. induction HT.
    (* Var *)
    apply sem_eval_well_defined_Var with (decls := decls)...
    (* Prod *)
    apply sem_eval_well_defined_Prod with (decls := decls)...
    (* Add *)
    apply sem_eval_well_defined_Add with (decls := decls)...
    (* Mul *)
    apply sem_eval_well_defined_Mul with (decls := decls)...
    apply sem_eval_well_defined_SMul with (decls := decls)...
    (* Transp *)
    apply sem_eval_well_defined_Transp with (decls := decls) (t := t)...
    (* Contr *)
    apply sem_eval_well_defined_Contr with (decls := decls) (d := d)...
    (* Proj *)
    apply sem_eval_well_defined_Proj with (decls := decls) (d := d)...
    (* Diag *)
    apply sem_eval_well_defined_Diag with (decls := decls)...
    (* Expand *)
    apply sem_eval_well_defined_Expand with (decls := decls)...
    (* Red *)
    apply sem_eval_well_defined_Red with (decls := decls) (d := d)...
Qed.
